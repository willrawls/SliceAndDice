...
//				this.mnuRulesCascadeDeletes.Checked = -1;
				...
				this.Text = "ODBC Database to Code Generator";
			...
			public void AddTable			{
				...
				End With;
				idxNew.Indexes.Append idxNew;

				if ( Len(sParentTable) > 0 )
				{;
				//  Attach to any indicated parent;
				idxNew.Fields.Append idxNew.CreateField(sParentTable + "ID", dbLong);
				idxnew = idxNew.CreateIndex(sParentTable + "IDIndex");
				...
				{;
				idxNew.Fields.Append idxNew.CreateField(sTableName + "Name", dbText);
				};
				if ( mnuRulesAutoAddDateCreated.Checked )
				{;
				idxNew.Fields.Append idxNew.CreateField("DateCreated", dbDate);
				};
				if ( mnuRulesAutoAddDateModified.Checked )
				{;
				idxNew.Fields.Append idxNew.CreateField("DateModified", dbDate);
				};
				End With;
...
{;
				...
				if ( Left(CurTable.Name, 4) <> "MSys" )
				{;
				if ( Right(CurTable.Fields(1).Name, 2) = "ID" )
				{;
				tvwTables.Nodes.Item(CurTable.Name).Parent = .Item(Left(CurTable.Fields(1).Name, Len(CurTable.Fields(1).Name) - 2));
				};
				};
				};
...
{;
 ...
 return;
				...
				if ( UCase(CurItem.Text) = UCase(.FieldName) )
				{;
...
{;
				...
				{;
				CurTable.Fields.Append CurTable.CreateField(.FieldName, NewField.dbFieldType, .Length);
				}
				else
				{;
...
{;
				...
				if ( Right(CurField.Name, 2) = "ID" )
				{;
				if ( Left(CurField.Name, Len(CurField.Name) - 2) = CurTable.Name )
				{;
				sIcon = "Key";
				}
				else
				...
				litX = lvwFields.ColumnHeaders.Add(, CurField.Name, CurField.Name, sIcon, sIcon);
