...
//				this.mnuSwitchTabsAutomatically.Checked = -1;
			...
			public  m_sTemplateDatabaseName;
			public string m_sCurrentEventResponseCategory;
			public  CurrentCodeArea;
			public  Parent;
			public  m_oDBClassGen;
			public  m_asaHistory;
			public  m_asaAttributes;
			public  SliceAndDice;
			public  CurrentTemplate;
			public  InternalCurrentTemplate;
			public  Complete;
			public  SadCommands();
			public  SadCommandSetCount;
			public  FavoriteCount;
			public  ExternalCount;
			public  CurrentHistoryEntry;
			public  ActionToDo;
			public  ActionParam;
			public  mbScramFormKey;
			public  mbFillingAddInScreen;
			public  mbIgnoreBlanks;
			public  mbIgnoreReadOnly;
			public  OkayToDoAction;
			public  FavoriteCalledFromIDE;
			public  OkayToUnload;
			public  WithEvents;
			public  Externals;
			public  CurrExternal;
...
{;
				...
				if ( InStr(UCase$(sExpression), " AND ") )
				{;
				...
				{if ( InStr(UCase$(sExpression), " XOR ") )
				{;
...
;
 ...
 return;
			...
			public As FillTemplateWithUserInput			{
				...
				Do While InStr(sGetToken(sToParse, 1, vbNewLine), gsSoftVarDelimiter) > 0    ' For each soft variable found;
				...
				For Curr = 1 To SadCommandSetCount;
				if ( SadCommands(CurrSet).ExecuteSoftCommandInline(asaX, UCase$(sVarName), sVar1, sNow) )
				{;
...
;
			...
			public void SetColors			{
				try
{;
				if ( Right$(BackColor, 1) = "+" )
				{
 BackColor = Left$(BackColor, Len(BackColor) - 1);
				if ( Right$(ForeColor, 1) = "+" )
				{
 ForeColor = Left$(ForeColor, Len(ForeColor) - 1);

				if ( Left$(BackColor, 2) <> "+H" )
				{
 BackColor = "+H" + BackColor;
				if ( Left$(ForeColor, 2) <> "+H" )
				{
...
;
			...
			public As sGetCurrentLineAtCharacter			{
				lCount = lTokenCount(Left$(sTextToSearch, lCharToStart), vbNewLine);
				if ( lCount > 0 )
				{;
...
;
 ...
 return;
				...
				if ( UCase$(sCurrentCategory) = "CHANGE FROM" )
				{;
...
;
			...
			public void mnuFileGenerateDeltaPatch_Click			{
				...
				if ( Len(sDate) )
				{;
				PatchFilename = App.Path + IIf(Right$(App.Path, 1) <> gsBS, gsBS, string.Empty) + "MDBPatch" + Replace(Format$(sDate, "00000.00"), gsP, "-") + ".sad";
...
;
			...
			public void mnuHelpSoftCommandReference_Click()
			{
				...
				{;
				MsgBox "No command set DLLs loaded." + gsEolTab + "No Soft Command Reference available." + gsEolTab + "Make sure S+D DLLs are in the same directory as the frmSplash.MDB you have loaded.";
			}
...
;
 ...
 return;
				...
				if ( Right$(sDatabasePath, 1) <> gsBS )
				{
 sDatabasePath = sDatabasePath + gsBS;
				if ( Right$(LCase$(snewDatabaseName), 4) <> ".mdb" )
				{
...
;
			...
			public void Form_Initialize()
			{
				...
				{;
				Name App.Path + gsBS + "SliceAndDiceNew.mdb" As App.Path + gsBS + "SliceAndDice.mdb";
			}
...
;
			...
			public void Form_Resize()
			{
				...
				tabCode.Height = ScaleHeight - tabCode.Top;
				// lsbJumpTo.Height = ScaleHeight - 415;
				if ( ScaleWidth - tabCode.Left < 0 )
				{
 return;       ' if ( there isn't enough display area to show the code entry areas, don't attempt to redraw it;
				tabCode.Width = ScaleWidth - tabCode.Left;
				txtName.Move lblCode(3).Left + lblCode(3).Width + 40, txtName.Top, tabCode.Width - (lblCode(3).Left + lblCode(3).Width + 40 - .Left), txtName.Height;
				txtShortName.Move lblCode(3).Left + lblCode(3).Width + 40, txtName.Top, tabCode.Width - (lblCode(3).Left + lblCode(3).Width + 40 - .Left), txtName.Height;

				txtCode(0).Move tabCode.Left + 100, .Top + 500, .Width - 200, .Height - 600;
				txtCode(1).Move tabCode.Left + 100, .Top + 500, .Width - 200, .Height - 600;
				txtCode(2).Move tabCode.Left + 100, .Top + 500, .Width - 200, .Height - 600;
				frmOptions.Move tabCode.Left + 100, .Top + 500, .Width - 200, .Height - 600;
				frmFile.Move tabCode.Left + 100, .Top + 500, .Width - 200, .Height - 600;
				...
				frmTemplateInfo.Move tabCode.Left + 100, .Top + 500, .Width - 200, .Height - 600;
...
{;
				...
				Parent.vbInst.ActiveCodePane.GetSelection lLine, lFirstCol, lLastLine, lLastCol    ' Determine where the cursor is;
				lLastLine = lLastLine - lLine + Abs(lLastCol > 1)    ' Determine what the last line selected is (discard last line if at beginning);
				sCode = Parent.vbInst.ActiveCodePane.CodeModule.Lines(lLine, lLastLine)   ' Grab the code selected from the active pane;
...
{;
				...
				Parent.vbInst.ActiveCodePane.GetSelection lLine, lFirstCol, lLastLine, lLastCol    ' Determine where the cursor is;
				lLastLine = lLastLine - lLine + Abs(lLastCol > 1)    ' Determine what the last line selected is (discard last line if at beginning);
				GetCurrentTextSelection = Parent.vbInst.ActiveCodePane.CodeModule.Lines(lLine, lLastLine)    ' Grab the code selected from the active pane;
				}
        catch(Exception e)
        {
            /* ON ERROR RESUME NEXT (ish) */
...
{;
				...
				Parent.vbInst.ActiveCodePane.GetSelection lLine, lFirstCol, lLastLine, lLastCol;
				lLastLine = lLastLine - lLine + Abs(lLastCol > 1);
				Parent.vbInst.ActiveCodePane.CodeModule.DeleteLines lLine, lLastLine;
				}
        catch(Exception e)
        {
            /* ON ERROR RESUME NEXT (ish) */
