...
//				this.lvwInfo1.NumItems = 0;
				...
				this.Text = "Sandle - Process C/C++ Header";
 ...
 sKey = sText;
				...
				tvwHierarchy.Nodes.Add(sParent,.ExpandedImage = sIcon;
				tvwHierarchy.Nodes.Add(sParent,.Expanded = bExpanded;
				tvwHierarchy.Nodes.Add(sParent,.BackColor = lvwContents.BackColor;
				tvwHierarchy.Nodes.Add(sParent,.ForeColor = lvwContents.ForeColor;
				tvwHierarchy.Nodes.Add(sParent,.Tag = sTag;
			}
			public void ProcessFile			{
				...
				{;
				sEOL = Chr(13);
				sRootName = Parent.Parent.sGetToken(sFilename, .lTokenCount(sFilename, "\"), "\");
				if ( bClearFirst )
				{;
...
;
			...
			public void tvwHierarchy_MouseUp			{
				...
				{
;
				if ( lvwContents.Tag <> UCase(CurrNode.Tag) )
				{;
				lvwContents.ListItems.Clear;
				switch UCase(CurrNode.Tag);
				Case "ASADEFINES";
				foreach( var CurrItem in asaDefines );

				lvwContents.ListItems.Add(,.SubItems(1) = CurrItem.Value;
			}
		}
}
