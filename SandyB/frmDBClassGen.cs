...
;
  ...
  db = OpenDatabase(msClassDatabaseName, dbDriverComplete, false, msClassDatabaseOptions);
			...
			public As RemoveNodes			{
				...
				if ( StrComp(UCase$(CurrNode.Image), UCase$(sNodeImageNameToRemove)) = 0 )
				{;
				...
				{
;
				sNodesLeft = sNodesLeft + CurrNode.Key + gsSC;
			}
...
{;
				...
				tvwTables.Nodes.Add(tvwTables.SelectedItem.Key,.ExpandedImage = "Table";
				tvwTables.Nodes.Add(tvwTables.SelectedItem.Key,.Expanded = true;
				}
        catch(Exception e)
        {
            /* ON ERROR RESUME NEXT (ish) */
