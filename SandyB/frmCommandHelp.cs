...
{;
				...
				sChoices = sChoices + CurrMember.SoftCommandName + IIf(CurrMember.IsInline, " (Inline)" + gsE + CurrMember.OneLineDescription + gsSC, gsE + CurrMember.OneLineDescription + gsSC);
				if ( Len(CurrMember.Aliases) )
				{;
				sChoices = sChoices + Replace(CurrMember.Aliases, ", ", IIf(CurrMember.IsInline, " (Inline)" + "=See " + CurrMember.SoftCommandName + gsSC, "=See " + CurrMember.SoftCommandName + gsSC));
				}
        catch(Exception e)
        {
            /* ON ERROR RESUME NEXT (ish) */
