...
Private Sub brwWebBrowser_NavigateComplete(ByVal URL As String)
    Dim CurrAddress As Integer
    Dim bFound      As Boolean

    Caption = brwWebBrowser.LocationName
    For CurrAddress = 0 To cboAddress.ListCount - 1
        If cboAddress.List(CurrAddress) = brwWebBrowser.LocationURL Then
           bFound = True
           Exit For
        End If
    Next CurrAddress
    ...
    DontNavigateNow = True
        If bFound Then cboAddress.RemoveItem CurrAddress
...
Public Sub GotoSite(SiteIndex As Long)
...
Public Sub LoadSites()
    ...
    With Sites
         ...
         .All = GetSetting("Slice And Dice", "Sandal", "AutoWeb Sites", sDefaults)

         If .Count = 0 Then
            .All = "0=Add a new site|||***ADD SITE***" & vbNewLine & _
                   "1=http://www.vbcode.com|||VBCode.com||Desc of VBCode.com|||" & vbNewLine & _
                   "2=http://www.planet-source-code.com|||Planet Source Code|||Desc of Planet Source Code|||" & vbNewLine
         End If
    End With
End Sub

Public Sub SaveSites()
    If Not Sites Is Nothing Then
