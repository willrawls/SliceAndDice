...
208      On Error GoTo EH_frmMain_FillAddInScreen
...
222          chkLocked = Abs(.Locked)
223          chkFavorite = Abs(.Favorite)
224          mnuIsFavorite.Checked = .Favorite
...
716                      sProcType = UCase$(sGetToken(II.sParam))    ' Get the procedure type (first of two parameters)                  [ Default: PROC ]
                     ElseIf IsNumeric(sGetToken(II.sParam, 3)) Then ' Three or more parameters passed
                         ...
                         II.ParamLineOffset = "" & sAfter(II.sParam)    ' Get the line offset (usually negative) (second of two parameters) [ Default: 0 ]
...
832                          Else
                            'Select Case II.SoftCommandName
                            '       Case "CODA"
                            '            II.OriginalCodeToInsert = Replace(II.OriginalCodeToInsert, "StartCoda", "STARTCODA")
                            '            II.OriginalCodeToInsert = Replace(II.OriginalCodeToInsert, "startcoda", "STARTCODA")
                            '            II.OriginalCodeToInsert = Replace(II.OriginalCodeToInsert, "STARTCODA", gsA, 1, 1)
                            '       Case "LOOPWHILE"
                            '            II.OriginalCodeToInsert = Replace(II.OriginalCodeToInsert, "StartLoopWhile", "STARTLOOPWHILE")
                            '            II.OriginalCodeToInsert = Replace(II.OriginalCodeToInsert, "startloopwhile", "STARTLOOPWHILE")
                            '            II.OriginalCodeToInsert = Replace(II.OriginalCodeToInsert, "STARTLOOPWHILE", gsA, 1, 1)
                            '       Case "LOOPUNTIL"
                            '            II.OriginalCodeToInsert = Replace(II.OriginalCodeToInsert, "StartLoopUntil", "STARTLOOPUNTIL")
                            '            II.OriginalCodeToInsert = Replace(II.OriginalCodeToInsert, "StartLoopUntil", "STARTLOOPUNTIL")
                            '            II.OriginalCodeToInsert = Replace(II.OriginalCodeToInsert, "STARTLOOPUNTIL", gsA, 1, 1)
                            'End Select
833                          End If

834                      Case "NOINSERT", "STOPCODEINSERTION"
835                          InternalInsertTemplate = True
836                          GoTo EH_InsertTemplate_Continue    ' Prematurely stop processing of this template

837                      Case "RESUMEINSERTION", "RESUME"  ' Clears file insertion and resume code insertion
838                          If Len(II.ExternalFilename) > 0 And Len(II.TextToSendToFile) > 0 Then
                              ' Check to see if we're supposed to be sending to the clipboard or keyboard instead of a file
                                ...
                                ElseIf II.ExternalFilename = "**MESSAGEWINDOW**" Then
                                   II.SoftVars("MESSAGEWINDOWOUTPUT").Value = II.TextToSendToFile
                                   II.LinesLeftToProcess = "~~ShowMessage MESSAGEWINDOWOUTPUT" & vbNewLine
                                Else
839                                On Error Resume Next      ' First save any results to a previously mentioned file.
840                                fh = FreeFile
841                                Open II.ExternalFilename For Append Access Write As #fh
842                                     Print #fh, II.TextToSendToFile
843                                Close #fh
                                End If

844                             II.TextToSendToFile = vbNullString
845                             Err.Clear
846                             On Error GoTo EH_InsertTemplate
847                          End If

848                          II.ExternalFilename = vbNullString
...
1411                         End If
                    End If
                         ...
                         Case "GOTOMESSAGE", "GOTOSHOWMESSAGE", "OUTPUTTOMESSAGE", "MESSAGEOUT", "MSGOUT"
                             II.ExternalFilename = "**MESSAGEWINDOW**"
                         ...
                         Case "GOTOKB", "GOTOKEYBOARD", "OUTPUTTOKEYBOARD", "OUTPUTTOKB", "OUTTOKB", "OUTTOKEYBOARD", "KEYOUT"
                             If UCase$(II.AllParameters) = "RAW" Or II.AllParameters = "1" Then
                                II.ExternalFilename = "**KEYBOARDRAW**"
                             Else
                                II.ExternalFilename = "**KEYBOARD**"
                             End If

                         Case "GOTOCLIPBOARD", "GOTOCLIP", "OUTPUTTOCLIPBOARD", "OUTTOCLIP", "OUTTOCLIPBOARD", "CLIPOUT"
                             II.ExternalFilename = "**CLIPBOARD**"
...
1525                     Case "FILENAME", "GOTOFILE", "OUTPUTTOFILE", "SENDTOFILE", "FILEOUT"      ' Set an external filename to output to
1526                         If Len(II.ExternalFilename) > 0 And Len(II.TextToSendToFile) > 0 Then
1527                             On Error Resume Next      ' First save any results to a previously mentioned file.
                                If Left$(II.ExternalFilename, 10) = "**KEYBOARD" Then
                                   If Right$(II.ExternalFilename, 5) = "RAW**" Then
                                      SendKeys Replace$(II.TextToSendToFile, vbNewLine, vbNullString)
                                   Else
                                ...
                                Else
1528                               fh = FreeFile
1529                               Open II.ExternalFilename For Append Access Write As #fh
1530                                 Print #fh, II.TextToSendToFile
1531                               Close #fh
                                End If
1532                            II.TextToSendToFile = vbNullString
1533                            Err.Clear
1534                            On Error GoTo EH_InsertTemplate
1535                         End If
1536                         II.ExternalFilename = II.AllParameters
...
1548                                 If SadCommands(CurrSet).ExecuteSoftCommand(II) Then
1549                                     Exit For
1550                                 End If
1551                             Next CurrSet
1552                         End If
...
1573     If Len(II.ExternalFilename) > 0 And Len(II.TextToSendToFile) > 0 Then
            If Left$(II.ExternalFilename, 10) = "**KEYBOARD" Then
               If Right$(II.ExternalFilename, 5) = "RAW**" Then
                  SendKeys Replace$(II.TextToSendToFile, vbNewLine, vbNullString)
               Else
            ...
            ElseIf II.ExternalFilename = "**MESSAGEWINDOW**" Then
               ...
               GoTo CODA_RESTART
            Else
1574           On Error Resume Next                          ' Save to file if previously mentioned and if there is code to save.
1575           Kill II.ExternalFilename
1576           fh = FreeFile
1577           Open II.ExternalFilename For Output Access Write As #fh
1578                Print #fh, II.TextToSendToFile
1579           Close #fh
1580           Err.Clear
            End If
1581     End If
...
2329         Exit Sub
2330     ElseIf SliceAndDice.Categorys(sCurrentCategory).CategoryType <> 0 Then
...
2521     If mbFillingAddInScreen Then Exit Sub
2522     chkFavorite = Abs(Not -chkFavorite)
2523     chkFavorite_Click
End Sub
...
2842   Form_KeyDown KeyCode, Shift
