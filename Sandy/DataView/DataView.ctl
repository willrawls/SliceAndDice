...
Public rmxData As New CAssocArray
...
Event OLEStartDrag(Data As MSComctlLib.DataObject, AllowedEffects As Long) 'MappingInfo=lvwMain,lvwMain,-1,OLEStartDrag
Event OLESetData(Data As MSComctlLib.DataObject, DataFormat As Integer) 'MappingInfo=lvwMain,lvwMain,-1,OLESetData
...
Event OLEDragOver(Data As MSComctlLib.DataObject, Effect As Long, Button As Integer, Shift As Integer, x As Single, y As Single, State As Integer) 'MappingInfo=lvwMain,lvwMain,-1,OLEDragOver
Event OLEDragDrop(Data As MSComctlLib.DataObject, Effect As Long, Button As Integer, Shift As Integer, x As Single, y As Single) 'MappingInfo=lvwMain,lvwMain,-1,OLEDragDrop
    ...
    lvwMain.Visible = False
        Set db = OpenDatabase(DatabaseName, , True)
            Set rst = db.OpenRecordset(RecordSource)
                ...
                End With
                FillListView rst, lvwMain, False
            rst.Close
            Set rst = Nothing
        db.Close
        Set db = Nothing
...
Public Sub FillListView(rst As Recordset, lvwCtrl As Object, Optional bFullLine As Boolean = True)
    ...
    Set lvwX = lvwCtrl
        ...
        Do Until rst.EOF
           ...
           For CurSubItem = 1 To rst.Fields.Count - 2
