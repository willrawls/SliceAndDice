...
' This is a friendly declaration of the CopyMemory function.  It is used to copy data into an EXTENSION_RECORD structure from a pointer to another structure.
...
Public Function GetExceptionText(ByVal ExceptionCode As Long) As String
  ...
  Select Case ExceptionCode
         ...
         Case Else:                               strExceptionString = "Unknown (&H" & Right$("00000000" & Hex(ExceptionCode), 8) & ")"
  End Select
...
'   It raises an error so that it can be trapped with an ON ERROR statement in the procedure that caused the exception.
...
' Call this function at the start of your program/DLL
Public Sub ExceptionStartup()
  Call SetUnhandledExceptionFilter(AddressOf MyExceptionFilter)
End Sub

' Call this function at the end of your program/DLL
' This step isn't necessary, but should be done for safety sake
Public Sub ExceptionShutdown()
