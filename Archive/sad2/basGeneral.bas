...
Public gsSoftVarDelimiter               As String ' "%%"
Public gsSoftCmdDelimiter               As String ' "~~"
Public gsInlineCmdDelimiter             As String ' "::"
Public gsNormalizeDelimiter             As String ' "%$%"
Public gsCategoryTemplateDelimiter      As String ' " - "
Public gsSpecialLineItemDelimiter       As String ' "**"
Public gsQueuedInsertionDelimiter       As String ' "~!~!"
Public gsSoftDirectiveDelimiter         As String ' "~##~"
...
Public Function lTokenCount(ByVal siAllTokens As String, Optional ByVal siDelim As String = gsS) As Long
    If Len(siDelim) = 0 Then
       lTokenCount = 1
    ElseIf Len(siAllTokens) = 0 Then
       lTokenCount = 0
    Else
       lTokenCount = UBound(Split(siAllTokens, siDelim)) + 1
    End If
...
'         If iToken < 1 Or Len(sDelim) = 0 Then Exit Function
'
'         Dim vX As Variant
'         vX = Split(siAllTokens, sDelim, iToken + 1)
'         sAfter = vX(iToken)
...
'         If iToken < 2 Or Len(sDelim) = 0 Then Exit Function
'         Dim vX As Variant
'         vX = Split(siAllTokens, sDelim)
'         iToken = iToken - 2
'         ReDim Preserve vX(0 To iToken)
'         sBefore = Join(vX, sDelim)
...
Public Sub InitPublic()
    ...
    gsSoftVarDelimiter = "%%"
    gsSoftCmdDelimiter = "~~"
    gsInlineCmdDelimiter = "::"
    gsNormalizeDelimiter = "%$%"
    gsCategoryTemplateDelimiter = " - "
    gsSpecialLineItemDelimiter = "**"
    gsQueuedInsertionDelimiter = "~!~!"
    gsSoftDirectiveDelimiter = "~##~"
End Sub
