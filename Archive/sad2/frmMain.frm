...
285                      If CurrAssocItem.Key = "sadRegister.NewCommands" Then
286                          SadCommands(SadCommandSetCount).CommandSet.Attributes("Registered").Value = True 'IIf(frmSplash.DetermineRegistration, "True", "False")
287                      End If
288                  End If
289              Else
...
Public Function ppInclude(ByVal sCode As String) As String
    Dim lStartLine          As Long
    Dim lAreaToInclude      As Long
    Dim sLinesToInclude     As String
    Dim sTemplateParameter  As String
    Dim sTemplateToInclude  As String
    Dim sCategoryName       As String
    Dim sTemplateName       As String
    Dim tTemplate           As CTemplate

    lStartLine = InStr(sCode, gsSoftDirectiveDelimiter & "Include ")
    Do While lStartLine > 0
        sTemplateParameter = sGetToken(Mid$(sCode, lStartLine + 12), 1, vbNewLine)
        sTemplateToInclude = sTemplateParameter
        If Val(sGetToken(sTemplateToInclude, 1, gsC)) > 0 Then
            lAreaToInclude = Val(sGetToken(sTemplateToInclude, 1, gsC))
            sTemplateToInclude = sAfter(sTemplateToInclude, 1, gsC)
        Else
            lAreaToInclude = 0
        End If
    
        If InStr(sTemplateToInclude, gsCategoryTemplateDelimiter) = 0 Then
            sCategoryName = sGetToken(InternalCurrentTemplate, 1, gsCategoryTemplateDelimiter)
            sTemplateName = sTemplateToInclude
        Else
            sCategoryName = sGetToken(sTemplateToInclude, 1, gsCategoryTemplateDelimiter)
            sTemplateName = sAfter(sTemplateToInclude, 1, gsCategoryTemplateDelimiter)
        End If
    
        sLinesToInclude = vbNullString
        If SliceAndDice.Categorys(sCategoryName) Is Nothing Then
           gbCancelInsertion = bUserSure("Preprocessor directive:" & vbNewLine & vbTab & gsSoftDirectiveDelimiter & "Include" & vbNewLine & vbNewLine & "Cannot find Category for Template:" & vbNewLine & vbTab & sCategoryName & " - " & sTemplateName & vbNewLine & vbNewLine & vbNewLine & "Cancel processing ?", "CANCEL PROCESSING ?")
           Set tTemplate = Nothing
        Else
           Set tTemplate = SliceAndDice.Categorys(sCategoryName).Templates(sTemplateName)
        End If
    
        If Not tTemplate Is Nothing Then
            With tTemplate
                 Select Case lAreaToInclude
                        Case 1: sLinesToInclude = .memoCodeAtTop
                        Case 2:    sLinesToInclude = .memoCodeAtCursor
                        Case 3:    sLinesToInclude = .memoCodeAtBottom
                        Case 0:    sLinesToInclude = .memoCodeAtTop
                        Case Else: sLinesToInclude = .memoCodeToFile
                  End Select
            End With
            Set tTemplate = Nothing
        Else
            gbCancelInsertion = bUserSure("Preprocessor directive:" & vbNewLine & vbTab & gsSoftDirectiveDelimiter & "Include" & vbNewLine & vbNewLine & "Cannot find Template:" & vbNewLine & vbTab & sCategoryName & " - " & sTemplateName & vbNewLine & vbNewLine & vbNewLine & "Cancel processing ?", "CANCEL PROCESSING ?")
            If gbCancelInsertion Then Exit Function
        End If
    
        sCode = Left$(sCode, lStartLine - 1) & sLinesToInclude & Mid$(sCode, lStartLine + 12 + Len(sTemplateParameter))
...
Public Function Preprocess(ByVal sCode As String) As String
    Dim vAreas          As Variant
    Dim vSubAreas       As Variant
    Dim vLines          As Variant
    Dim vPrevReferences As Variant
    Dim lAreaCount      As Long
    Dim lCurrArea       As Long
    Dim lCurrReference  As Long
    Dim lCurrLine       As Long
    Dim lLineCount      As Long
    Dim NextElse        As Long
    Dim NextEndIf       As Long
    Dim sIndent         As String
    Dim bLeaveComments  As Boolean
    
    If Len(sCode) > 0 Then
        If InStr(sCode, gsSoftDirectiveDelimiter) Then
            sCode = ppInclude(sCode)
    
            vAreas = Split(sCode, gsSoftDirectiveDelimiter)
            lAreaCount = UBound(vAreas)
            For lCurrArea = 1 To lAreaCount
                Select Case UCase$(sGetToken(sGetToken(vAreas(lCurrArea), 1, vbNewLine)))
                    Case "SOFTCODE", "BEGINSOFTCODE", "SOFTCOMMANDS", "BEGINSOFTCOMMANDS", "COMMANDS", "BEGINCOMMANDS", "SCRIPT", "BEGINSCRIPT", "CODE", "BEGINCODE"
                        vLines = Split(vAreas(lCurrArea), vbNewLine)
                        lLineCount = UBound(vLines)
                        For lCurrLine = 1 To lLineCount
                            If Len(vLines(lCurrLine)) > 0 Then
                                vLines(lCurrLine) = gsSoftCmdDelimiter & LTrim$(vLines(lCurrLine))
                            End If
                        Next
                        vAreas(lCurrArea) = Replace$(sAfter(Join(vLines, vbNewLine), 1, vbNewLine), gsSoftCmdDelimiter & "!!", vbNullString)

                    Case "MIXEDMODE", "NORMAL", _
                         "ENDSOFTCODE", "ENDSOFTCOMMANDS", _
                         "ENDCOMMANDS", "ENDSCRIPT", "ENDCODE", _
                         "ENDCOMMENTS", "ENDNOTES"
                        vAreas(lCurrArea) = sAfter(vAreas(lCurrArea), 1, vbNewLine)

                    Case "LEAVECOMMENTS"
                        bLeaveComments = True

                    Case "COMMENTS", "BEGINCOMMENTS", "NOTES", "BEGINNOTES"
                        vAreas(lCurrArea) = gsSoftCmdDelimiter & gsA

                    Case "OVERRIDEDELIMITER", "DELIMITER"
                        
                End Select
            Next
            sCode = Join(vAreas, vbNullString)
        End If
    
        If Not bLeaveComments Then
            vLines = Split(sCode, vbNewLine)
            vLines = Filter(vLines, gsSoftCmdDelimiter & gsA, False)
            sCode = Join(vLines, vbNewLine)
        End If
        
        'Auto add CommandReferences to ~~If..~~Else..~~EndIf structures
        vAreas = Split(sCode, gsSoftCmdDelimiter & "If")
        If UBound(vAreas) > 0 Then
            sCode = vAreas(0)
            vPrevReferences = Array()
            For lCurrArea = 1 To UBound(vAreas)
                If UBound(vPrevReferences) > -1 Then
                    vPrevReferences = Split(Join(vPrevReferences, ",") & "," & lCurrArea, ",")
                Else
                    vPrevReferences = Array(lCurrArea)
                End If
                NextEndIf = InStr(vAreas(lCurrArea), gsSoftCmdDelimiter & "EndIf")
                NextElse = InStr(vAreas(lCurrArea), gsSoftCmdDelimiter & "Else")
                Do While NextEndIf + NextElse
                    If NextElse < NextEndIf Or NextEndIf = 0 Then
                        vSubAreas = Split(vAreas(lCurrArea), gsSoftCmdDelimiter & "Else", 2)
                        If UBound(vSubAreas) > 0 Then
                            vAreas(lCurrArea) = Join(vSubAreas, gsSoftCmdDelimiter & "_i" & vPrevReferences(UBound(vPrevReferences)) & "_Else")
                        End If
                    End If
                    If NextEndIf > 0 Then
                        vSubAreas = Split(vAreas(lCurrArea), gsSoftCmdDelimiter & "EndIf", 2)
                        If UBound(vSubAreas) > 0 Then
                            vAreas(lCurrArea) = Join(vSubAreas, gsSoftCmdDelimiter & "_i" & vPrevReferences(UBound(vPrevReferences)) & "_EndIf")
                            vPrevReferences = Split(sBefore(Join(vPrevReferences, ","), lTokenCount(Join(vPrevReferences, ","), ","), ","), ",")
                        End If
                    End If
                    NextEndIf = InStr(vAreas(lCurrArea), gsSoftCmdDelimiter & "EndIf")
                    NextElse = InStr(vAreas(lCurrArea), gsSoftCmdDelimiter & "Else")
                Loop
                sCode = sCode & gsSoftCmdDelimiter & "_i" & lCurrArea & "_If" & vAreas(lCurrArea)
            Next lCurrArea
        End If
        
        AddCommandReferences sCode, "StartCoda", "Coda", "c"
        AddCommandReferences sCode, "StartLoopWhile", "LoopWhile", "w"
        AddCommandReferences sCode, "StartLoopUntil", "StartLoopUntil", "u"
    End If
    
    Preprocess = sCode
End Function

Public Sub AddCommandReferences(ByRef sCode As String, ByVal StartName As String, ByVal EndName As String, ByVal Prefix As String)
    Dim vAreas          As Variant
    Dim vSubAreas       As Variant
    Dim vPrevReferences As Variant
    Dim lCurrArea       As Long
    ...
    vAreas = Split(sCode, gsSoftCmdDelimiter & StartName)
    If UBound(vAreas) > 0 Then
        sCode = vAreas(0)
        vPrevReferences = Array()
        For lCurrArea = 1 To UBound(vAreas)
            ...
            Else
                vPrevReferences = Array(lCurrArea)
            End If
            NextEndIf = InStr(vAreas(lCurrArea), gsSoftCmdDelimiter & EndName)
            Do While NextEndIf
                vSubAreas = Split(vAreas(lCurrArea), gsSoftCmdDelimiter & EndName, 2)
                If UBound(vSubAreas) > 0 Then
                    vAreas(lCurrArea) = Join(vSubAreas, gsSoftCmdDelimiter & "_c" & vPrevReferences(UBound(vPrevReferences)) & "_" & EndName)
                    vPrevReferences = Split(sBefore(Join(vPrevReferences, ","), lTokenCount(Join(vPrevReferences, ","), ","), ","), ",")
                End If
                NextEndIf = InStr(vAreas(lCurrArea), gsSoftCmdDelimiter & EndName)
            Loop
            sCode = sCode & gsSoftCmdDelimiter & "_" & Prefix & lCurrArea & "_" & StartName & vAreas(lCurrArea)
        Next lCurrArea
    End If
End Sub
...
435          asaVar("Module Name") = .CodeModule.Parent.Name
             Select Case .CodeModule.Parent.Type
             Case vbext_ct_StdModule:              asaVar("Module Type") = "MODULE"
             Case vbext_ct_ClassModule:       asaVar("Module Type") = "CLASS"
             Case vbext_ct_MSForm, 5:         asaVar("Module Type") = "FORM"
             Case vbext_ct_ActiveXDesigner:   asaVar("Module Type") = "DESIGNER"
             Case 100:                        asaVar("Module Type") = "DOCUMENT"
             Case Else:                       asaVar("Module Type") = "OTHER"
             End Select
...
608      Dim scT4 As String
         ...
         InitPublic     ' Resets all delimiters to their defaults
...
614         Set CurModule = Parent.vbInst.ActiveCodePane.CodeModule
        ...
        II.OriginalCodeToInsert = Preprocess(II.OriginalCodeToInsert)
        If gbCancelInsertion Then GoTo EH_InsertTemplate_Continue
...
744                          On Error Resume Next
                                ...
                                Dim CurItem         As CAssocItem
                                For Each CurItem In II.SoftVars
                                    If (InStr(CurItem.Value, vbNewLine) = 0) And (CurItem.Key <> "DEBUG") Then
                                        sAllKeyValues = sAllKeyValues & CurItem.Key & gsE & CurItem.Value & vbNewLine
                                    End If
                                Next CurItem
                                sAllKeyValues = sAllKeyValues & "-["
                                For Each CurItem In II.SoftVars
                                    If (InStr(CurItem.Value, vbNewLine) > 0) And (CurItem.Key <> "DEBUG") And (Left$(CurItem.Key, 3) <> "EOL") Then
                                        sAllKeyValues = sAllKeyValues & CurItem.Key & "]" & String$(Abs(77 - Len(CurItem.Key)), "-") & vbNewLine & CurItem.Value & IIf(Right$(CurItem.Value, 2) = vbNewLine, vbNullString, vbNewLine) & String$(80, "-") & vbNewLine & "-["
                                    End If
                                Next CurItem
                                sAllKeyValues = Left$(sAllKeyValues, Len(sAllKeyValues) - 4)
745                          II.SoftVars("DEBUG").Value = _
                                "Template Name=" & II.TemplateName & vbNewLine & _
                                "Point Of Insertion=" & II.PointOfInsertion & vbNewLine & _
                                "-[Soft Variables]" & String$(80 - 17, "-") & vbNewLine & sAllKeyValues & vbNewLine & _
                                "-[Lines Left To Process]" & String$(80 - 24, "-") & vbNewLine & II.LinesLeftToProcess & vbNewLine & String$(80, "-") & vbNewLine & _
                                "-[Original Code To Insert]" & String$(80 - 26, "-") & vbNewLine & II.OriginalCodeToInsert & vbNewLine & String$(80, "-") & vbNewLine
...
762                              NextElse = InStr(UCase$(II.LinesLeftToProcess), gsSoftCmdDelimiter & UCase$(CommandReference) & "ELSE")
...
799                              bDoCoda = (Val(Evaluate(II.AllParameters, II.SoftVars)) = 0)
800                          Else
801                              bDoCoda = (Val(Evaluate(II.AllParameters, II.SoftVars)) <> 0)
802                          End If
803                          If bDoCoda Then
                            Select Case II.SoftCommandName
                                Case "CODA"
...
1132                         Err.Clear
                             sProcType = UCase$(sGetToken(II.sParam, 1))
...
1199                         II.SoftVars(II.AllParameters).Value = Clipboard.GetText(vbCFText)
...
1352                         II.SoftVars("Module Name").Value = Parent.vbInst.ActiveCodePane.CodeModule.Parent.Name
                             Select Case Parent.vbInst.ActiveCodePane.CodeModule.Parent.Type
                             Case vbext_ct_StdModule:         II.SoftVars("Module Type").Value = "MODULE"
                             Case vbext_ct_ClassModule:       II.SoftVars("Module Type").Value = "CLASS"
                             Case vbext_ct_MSForm, 5:         II.SoftVars("Module Type").Value = "FORM"
                             Case vbext_ct_ActiveXDesigner:   II.SoftVars("Module Type").Value = "DESIGNER"
                             Case 100:                        II.SoftVars("Module Type").Value = "DOCUMENT"
                             Case Else:                       II.SoftVars("Module Type").Value = "OTHER"
                             End Select
...
1368                                         .Activate
1369                                     End With
                                         II.SoftVars("Module Type").Value = "CLASS"
1370                                 Case "FORM"
...
On Error Resume Next
                             ...
                             If Err.Number <> 0 Then
                                 If InStr(sProcName, "_") Then
                                     Err.Clear
                                     II.PointOfInsertion = CurModule.CreateEventProc(sGetToken(sProcName, 2, "_"), sGetToken(sProcName, 1, "_")) + IIf(II.ParamLineOffset = 0, 1, II.ParamLineOffset)
                                     If Err.Number <> 0 Then
                                         MsgBox "GOTOPROC" & gsEolTab & "Can't find:" & gsEolTab & vbTab & sProcName & gsEolTab & "In module:" & gsEolTab & vbTab & CurModule.Parent.Name
...
1395                         If (Len(sProcName) > 0) And (Not gbCancelInsertion) Then
...
1880                     End With
                         sOpened = sTemplateIcon(CurrCategory.Templates("Property - Guid"))
                         With .Add(CurrCategory.Key & " - Collection Member", tvwChild, CurrCategory.Key & " - Property - Guid", "Property - Guid", sOpened, sOpened)
                             .ExpandedImage = sOpened
                             .Expanded = True
                             .ToolTip = "Inserted for each Guid field."
                         End With
...
1937                                                 .ExpandedImage = sOpened
                                                     .ToolTip = "User template. Probably used by """ & gsSoftDirectiveDelimiter & "Include"" from one of the standard data-pasting templates."
1938                                             End With
1939                                         End If
1940                                 End Select
1941                             End If
...
2848     InitPublic
         NextNegativeUnique                           ' Sets first time to -2 vs -1 (since -1 is usually used to indicated that a new negative # is needed)
...
2894     On Error Resume Next
         Dim OldDialogTitle As String
2895     Err.Clear
2896     With cdgSelect
2897         .Filter = "Access Database (*.mdb)|*.mdb|All Files (*.*)|*.*"
2898         .FilterIndex = 0
             OldDialogTitle = .DialogTitle
             .DialogTitle = "Please select an Access Database"
...
3020                             sSelectedText = sGetToken(sGetToken(sSelectedText, 2, gsSoftVarDelimiter), 1, gsInlineCmdDelimiter) & "*I"
                             ElseIf InStr(sSelectedText, gsSoftDirectiveDelimiter) Then
