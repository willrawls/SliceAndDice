...
{;
                ...
                if ( StrComp(sCategory, gsSpecialLineItemDelimiter + "CURRENT " + gsCategory) + gsSpecialLineItemDelimiter, vbTextCompare) = 0 .ToUpper()
            {;
...
{;
 ...
 Exit For;
            ...
            {;
                if ( ! bUserSure("Please set up a 'Change to' " + gsTemplate + " named" + gsEolTab + "'Change from" + gsCategoryTemplateDelimiter + CurControl.ClassName + gsA + vbNewLine + "before using this function on the '" + CurControl.ClassName + "' type control." + gs2EOL + "Select Yes to create the new " + gsTemplate + gsP + vbNewLine + "Select No to abort.") )
            {;
                return;
                }
        catch(Exception e)
